...
class DevConsumptionManagementModal {
    ...
    showHelp(){
        ...
        this.alert.textContent = 'Wenn es Probleme gibt, meldet euch bitte bei Laurin Domenig in LeLa 6.';
        this.alert.style.display = 'block';
    }
...
class DevUserManagementModal {
    ...
    displayUsers(users) {
        ...
        const visibleCount = users.filter(u => u.status).length;
        const hiddenCount = users.filter(u => !u.status).length;
        const visibleCountEl = document.getElementById('visibleUsersCount');
        const hiddenCountEl = document.getElementById('hiddenUsersCount');
        if (visibleCountEl) visibleCountEl.textContent = `Visible: ${visibleCount}`;
        if (hiddenCountEl) hiddenCountEl.textContent = `Hidden: ${hiddenCount}`;
        ...
        users.forEach(user => {
            ...
            // For roles 4 and 5: show help instead of PIN controls
            const isHelpOnlyRole = user.role_id === 4 || user.role_id === 5;
            const pinControls = isHelpOnlyRole
              ? `<button class="btn btn-outline-info btn-sm me-2" onclick="devUserManagementModal.showHelp()" title="Hilfe">
                    <i class='fas fa-info-circle'></i> Hilfe
                 </button>`
              : (user.has_pin
            ...
            const statusButton = user.status
              ? `<button class="btn btn-outline-warning btn-sm me-2" onclick="devUserManagementModal.toggleUserStatus(${user.id}, '${user.first_name} ${user.last_name}')" title="Hide User">
                   <i class="fas fa-eye-slash"></i> Hide
                 </button>`
              : `<button class="btn btn-outline-success btn-sm me-2" onclick="devUserManagementModal.toggleUserStatus(${user.id}, '${user.first_name} ${user.last_name}')" title="Show User">
            ...
            userItem.innerHTML = `
                <div class="flex-fill">
                    <div class="manage-user-row row g-2 align-items-center">
                        <div class="col-md-2 col-12 name-field">
                        ...
                        <div class="col-md-2 col-12 role-select">
                            <select class="form-select form-select-sm" id="role_${user.id}"><option>Loading roles...</option></select>
                        </div>
                        <div class="col-md-2 col-12 status-section">
                            ${statusBadge}
                        </div>
                        <div class="col-md-4 col-12 d-flex justify-content-end actions">
    ...
    async toggleUserStatus(userId, userName){
        try{
            const res = await fetch(`/dev/toggle_user_status/${userId}`, { method: 'POST' });
            const data = await res.json();
            if(res.ok && data.success){
                this.showAlert(data.message, 'success');
                this.loadUsers();
            } else {
                this.showAlert(data.error || 'Failed to toggle user status', 'danger');
            }
        } catch(e){
